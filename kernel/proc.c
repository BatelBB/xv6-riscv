...
int setAccumulator(void)
{
  ...
  for (i = 0; i < NPROC && !found; i++)
  {
    ...
    {
      min = proc[i].accumulator;
      found = 1;
      for (j = i; j < NPROC; j++)
      {
        if ((proc[j].state == RUNNABLE || proc[j].state == RUNNING) && min > proc[j].accumulator)
          min = proc[j].accumulator;
      }
    }
  }
  return min;
    ...
    for (pp = proc; pp < &proc[NPROC]; pp++)
    {
      if (pp->parent == p)
      {
        ...
        if (pp->state == ZOMBIE)
        {
          ...
          if (msg != 0 && copyout(p->pagetable, msg, pp->exit_msg, sizeof(pp->exit_msg)) < 0)
          {
