...
fork(void)
{
  ...
  // Allocate process.
  if((np = allocproc()) == 0){    //holding np->lock 
    return -1;
  }
  ...
  *(np->kthread[0].trapframe) = *(kt->trapframe);
  ...
  pid = np->pid;
  release(&np->kthread[0].lock);        //check
...
yield(void)
{
  ...
  // struct proc *p = myproc();         //TODO: maybe need to hold this lock
  struct kthread* kt = mykthread();
